...
function getNormalCategories(platform) {
  return platform === 'wechat'
    ...
    : Object.entries(CATEGORIES)
      .filter(([name, cat]) => name !== '热文风格' && !cat.wechatOnly)
      .map(([name]) => name);
}
...
[{"topic": "标题", "category": "热文风格", "main_character": "核心角色", "characters": ["角色1", "角色2"], "related_works": ["作品1", "作品2"]}]`;
  ...
  if (normalCount > 0) {
    ...
    const topicSection = `
【标题类型说明】
以下是可用标题句式库。你必须结合上面的"指定类别"选最匹配的句式，整批标题必须分散使用，禁止套用固定模板：

类型1 - 二选一悬念：
- 标题抛出两个可能的原因或动机，引发读者好奇
- 可以是问句也可以是陈述句
- 示例句式："XX这么做，是A还是B""XX没有出手，不是怕而是另有盘算"

类型2 - 具体细节挖掘：
- 聚焦一个具体情节或数字，揭示背后真相
- 标题要有画面感和具体细节
- 示例句式："XX临走前那句话，藏着后来的结局""XX袖中那件小物，才是整段暗线"
...
- 表面强的在某方面弱，表面弱的在某方面强
- 用"却""偏偏""竟"等转折词制造反差
- 示例句式："看似最软弱的XX，反而看穿了XX""XX赢了局面，却输掉人心"
